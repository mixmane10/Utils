...
Path.StatusType = { Idle = "Idle", Active = "Active" }
Path.ErrorType = { LimitReached = "LimitReached", TargetUnreachable = "TargetUnreachable", ComputationError = "ComputationError", AgentStuck = "AgentStuck" }

-- Visual waypoint template
...
local function createVisualWaypoints(waypoints)
	...
	for i, wp in ipairs(waypoints) do
		...
		clone.Color = (i==#waypoints and Color3.fromRGB(0,255,0)) or (wp.Action==Enum.PathWaypointAction.Jump and Color3.fromRGB(255,0,0)) or Color3.fromRGB(255,139,0)
...
-- Constructor
function Path.new(agent, agentParams, override)
	if not (agent and agent:IsA("Model") and agent.PrimaryPart) then
		output(error, "Agent must be a Model with a PrimaryPart.")
	end
...
function Path:Run(target)
	...
	destroyVisualWaypoints(self._visualWaypoints)
	if self._settings.VISUALIZE then
...
function Path:Destroy()
	self:Stop()
	for _, ev in pairs(self._events) do ev:Destroy() end
	self._path:Destroy()
	for k,_ in pairs(self) do self[k] = nil end
	setmetatable(self, nil)
end

function Path.GetNearestCharacter(fromPos)
	local nearest, dist = nil, math.huge
	for _, p in pairs(Players:GetPlayers()) do
		if p.Character and p.Character.PrimaryPart then
			local d = (p.Character.PrimaryPart.Position - fromPos).Magnitude
			if d < dist then
